...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Whether to validate the remote build functionality.",
    )
    ...
    logging.debug(
        f"Repo path: {args.repo_path} validate_paths: {args.validate_paths} validate_azd: {args.validate_azd} debug: {args.debug} topics: {args.topics} expected_topics: {args.expected_topics} validate_playwright_test: {args.validate_playwright_test} validate_remote_build: {args.validate_remote_build} psrule: {args.psrule_result} output: {args.output}"
    )
